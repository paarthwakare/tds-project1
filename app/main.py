...
def process_request(data):
    ...
    if data["round"] == 1:
        ...
        # If Pages API reported enabled, verify the site is reachable
        if pages_ok and pages_url:
            from app.github_utils import wait_for_pages
            reachable = wait_for_pages(pages_url, timeout=120)
            if not reachable:
                print("⚠ Pages enabled but site not reachable yet:", pages_url)
    else:
    ...
    notified = notify_evaluation_server(data["evaluation_url"], payload)

    processed = load_processed()
    key = f"{data['email']}::{data['task']}::round{round_num}::nonce{data['nonce']}"
    processed_entry = payload.copy()
    processed_entry["notified"] = bool(notified)
    processed_entry["notified_at"] = datetime.utcnow().isoformat()
    processed[key] = processed_entry
