...
def wait_for_pages(pages_url: str, timeout: int = 120) -> bool:
    """
    Poll the pages_url until it returns HTTP 200 or timeout (seconds) elapses.
    Returns True if 200 OK observed, False otherwise.
    """
    import time
    elapsed = 0
    interval = 2
    try:
        while elapsed < timeout:
            try:
                r = httpx.get(pages_url, timeout=10.0)
                if r.status_code == 200:
                    print("✅ Pages site reachable:", pages_url)
                    return True
                else:
                    print("Pages site not ready yet (status):", r.status_code)
            except Exception as e:
                print("Pages site check failed:", e)
            time.sleep(interval)
            elapsed += interval
    except KeyboardInterrupt:
        pass
    print("❌ Pages site did not become reachable within timeout:", pages_url)
    return False
